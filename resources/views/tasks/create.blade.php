...
<div class="max-w-3xl mx-auto px-4 sm:px-6">
    <div class="mb-6">
        ...
        <h1 class="text-3xl font-bold text-white">Buat Task Baru</h1>
    </div>

    <form method="POST" action="{{ route('tasks.store', $team->id) }}" class="space-y-6 bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-sm">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="responsible_member_id"
                    ...
                    <option value="{{ $member->id_member }}" @selected(old('responsible_member_id')==$member->id_member)>
                        {{ $member->user->name }}
                    </option>
                    @endforeach
                </select>
        ...
        <div class="flex gap-3 pt-4">
            <button type="submit" id="submit-btn" class="flex-1 px-6 py-3 rounded-lg bg-cyan-500 hover:bg-cyan-600 transition text-white font-semibold">
                <i class="fa-solid fa-check mr-2"></i> Buat Task
            </button>
            <a href="{{ route('tasks.index', $team->id) }}" class="flex-1 px-6 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition text-white font-semibold text-center">
