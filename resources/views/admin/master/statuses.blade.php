            ...
            @forelse($statuses as $status)
                <div class="p-4 rounded-xl bg-slate-900/50 border border-white/10 space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold {{ $status->is_default ? 'bg-emerald-500/20 text-emerald-100' : 'bg-slate-700 text-slate-200' }}">
                            {{ $status->is_default ? 'Default' : 'Non-default' }}
                        </span>
                    </div>
                    <div class="flex items-center gap-2 text-xs text-slate-200">
                        <span class="h-4 w-4 rounded-full" style="background: {{ $status->color }}"></span>
                        <span>{{ $status->color }}</span>
                        <span class="ml-auto">Weight: {{ $status->weight }}</span>
                    </div>
                    <form method="POST" action="{{ route('admin.master.statuses.update', $status) }}" class="space-y-2">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-2 gap-2">
                            <input type="text" name="label" value="{{ $status->label }}" class="rounded-lg border border-slate-200/40 bg-slate-950/70 px-2 py-2 text-xs text-slate-100 focus:ring-2 focus:ring-cyan-500" placeholder="Label">
                            <input type="color" name="color" value="{{ $status->color }}" class="h-10 w-full rounded border border-slate-200/40">
                            <input type="number" name="weight" value="{{ $status->weight }}" min="0" max="255" class="w-full rounded-lg border border-slate-200/40 bg-slate-950/70 px-2 py-2 text-xs text-slate-100 focus:ring-2 focus:ring-cyan-500" placeholder="Weight">
                    ...
                    <form method="POST" action="{{ route('admin.master.statuses.destroy', $status) }}">
                        @csrf
                        @method('DELETE')
