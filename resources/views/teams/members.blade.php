        ...
        <div class="space-y-6">
            ...
            <div class="bg-slate-900 text-slate-50 rounded-2xl shadow p-6 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-xs px-2 py-1 rounded-full bg-white/10 text-white">{{ $roleStats->count() }} role</span>
                </div>
                <div class="space-y-3">
                    @foreach (['owner' => 'Owner', 'member' => 'Member'] as $key => $label)
                    <div class="flex items-center justify-between p-3 rounded-xl border border-slate-800 bg-slate-800/60">
                        <div>
                            <p class="text-sm font-semibold">{{ $label }}</p>
                            <p class="text-xs text-slate-300">{{ $roleStats[$key] ?? 0 }} anggota</p>
                        </div>
                    </div>
                    @endforeach
                </div>
                <a href="{{ route('teams.settings', $team) }}" class="w-full inline-flex items-center justify-center px-4 py-2.5 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold shadow hover:from-cyan-600 hover:to-blue-700 transition">
                    Kelola role di Pengaturan
                </a>
            </div>
        </div>
    </div>
